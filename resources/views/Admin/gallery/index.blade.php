...
@section('title', 'System Gallery Management')
...
@section('content')
    <div class="space-y-8 animate-fade-in">
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
                ...
                <p class="font-medium" style="color: var(--text-secondary);">Manage all visual media uploaded by admins and mentors.</p>
            </div>
            <div class="flex items-center gap-3">
                ...
                <a href="{{ route('admin.gallery.create') }}" class="btn btn-primary shadow-lg shadow-[#0B4D73]/20">
        ...
        <div class="grid grid-cols-2 md:grid-cols-2 lg:grid-cols-4 gap-6">
            <div class="analytics-card rounded-2xl p-6 relative overflow-hidden group">
                <div class="relative z-10">
                    ...
                    <p class="text-[10px] sm:text-sm font-bold uppercase tracking-wider mb-1" style="color: var(--text-secondary);">Total Media</p>
                    <h3 class="text-2xl sm:text-3xl font-black" style="color: var(--text-primary);">{{ $images->total() }}</h3>
                </div>
            </div>
            ...
            <div class="analytics-card rounded-2xl p-6 relative overflow-hidden group">
                <div class="relative z-10">
                    ...
                    <p class="text-[10px] sm:text-sm font-bold uppercase tracking-wider mb-1" style="color: var(--text-secondary);">Active</p>
                    <h3 class="text-2xl sm:text-3xl font-black text-emerald-500">{{ $images->where('status', 'active')->count() }}</h3>
                </div>
            </div>
            ...
            <div class="analytics-card rounded-2xl p-6 relative overflow-hidden group">
                <div class="relative z-10">
                    ...
                    <p class="text-[10px] sm:text-sm font-bold uppercase tracking-wider mb-1" style="color: var(--text-secondary);">Featured</p>
                    <h3 class="text-2xl sm:text-3xl font-black text-amber-500">{{ $images->where('is_featured', true)->count() }}</h3>
                </div>
            </div>
            ...
            <div class="analytics-card rounded-2xl p-6 relative overflow-hidden group">
                <div class="relative z-10">
                    ...
                    <p class="text-[10px] sm:text-sm font-bold uppercase tracking-wider mb-1" style="color: var(--text-secondary);">Categories</p>
                    <h3 class="text-2xl sm:text-3xl font-black text-purple-600">{{ $images->unique('category')->count() }}</h3>
                </div>
            </div>
        </div>
        ...
        <div id="grid-view" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            @forelse($images as $image)
                <div class="glass rounded-3xl overflow-hidden group border border-slate-100/50 hover:shadow-xl hover:-translate-y-1 transition-all duration-300 flex flex-col">
                    <div class="aspect-video relative overflow-hidden bg-slate-100">
                        ...
                        <div class="absolute top-3 left-3 flex flex-col gap-2">
                            <span class="px-2 py-1 rounded-lg bg-white/90 backdrop-blur-sm text-[9px] font-black uppercase text-slate-800 border border-white/20 shadow-sm self-start">
                                {{ $image->category }}
                            </span>
                            <span class="px-2 py-1 rounded-lg bg-[#0B4D73]/90 backdrop-blur-sm text-[9px] font-black uppercase text-white border border-white/10 shadow-sm self-start">
                                by {{ $image->author->first_name }}
                            </span>
                        </div>

                        @if($image->is_featured)
                            <div class="absolute top-3 right-3">
                                <div class="w-8 h-8 rounded-full bg-amber-400 text-white flex items-center justify-center shadow-lg shadow-amber-400/30">
                    ...
                    <div class="p-4 flex-1 flex flex-col">
                        ...
                        <div class="mt-auto flex items-center justify-between border-t border-slate-50 pt-3">
                            <span class="inline-flex items-center px-2 py-0.5 rounded-full text-[9px] font-black uppercase border {{ $image->status === 'active' ? 'bg-emerald-50 text-emerald-600 border-emerald-100' : 'bg-slate-50 text-slate-400 border-slate-100' }}">
                                {{ $image->status }}
                            </span>
                            <span class="text-[9px] font-bold text-slate-400 uppercase tracking-widest">{{ $image->created_at->format('M d, Y') }}</span>
                        </div>
                    </div>
                </div>
            @empty
                <div class="col-span-full glass rounded-3xl p-24 text-center border-2 border-dashed border-slate-200">
                    ...
                    <h3 class="text-2xl font-bold text-slate-900 mb-2">No Media Found</h3>
                    <p class="text-slate-500">Your gallery artifacts will appear here once uploaded.</p>
                </div>
            @endforelse
        </div>
        ...
        <div id="table-view" class="hidden overflow-x-auto glass rounded-3xl border border-slate-100/50">
            <table class="w-full text-left border-collapse">
                ...
                <tbody class="divide-y divide-slate-50">
                        ...
                        <tr class="hover:bg-slate-50/50 transition-colors group">
                            <td class="px-6 py-4">
                                <div class="flex items-center gap-4">
                                    ...
                                    <div>
                                        <p class="font-bold text-slate-900 leading-none mb-1 group-hover:text-[#0B4D73] line-clamp-1">{{ $image->title }}</p>
                                        <p class="text-[10px] text-slate-400 font-medium">{{ $image->created_at->format('M d, Y') }}</p>
                                    </div>
                                </div>
                            </td>
                            <td class="px-6 py-4">
                                <span class="px-2 py-1 rounded-md bg-slate-100 text-slate-600 text-[9px] font-black uppercase tracking-widest">{{ $image->category }}</span>
                            </td>
                            <td class="px-6 py-4">
                                <span class="text-xs font-bold text-slate-700">{{ $image->author->name }}</span>
                            </td>
                            <td class="px-6 py-4 text-center">
                                <span class="px-2 py-0.5 rounded-full text-[9px] font-black uppercase tracking-widest border {{ $image->status === 'active' ? 'bg-emerald-50 text-emerald-600 border-emerald-100' : 'bg-slate-50 text-slate-400 border-slate-100' }}">
                                    {{ $image->status }}
                                </span>
                            </td>
                            <td class="px-6 py-4 text-right">
                                <div class="flex items-center justify-end gap-2">
                                    <a href="{{ route('admin.gallery.edit', $image->id) }}" class="p-2 text-slate-400 hover:text-amber-500 hover:bg-amber-50 rounded-lg transition-all">
                                        <i class="fas fa-edit text-sm"></i>
                                    </a>
                                    <form action="{{ route('admin.gallery.destroy', $image->id) }}" method="POST" class="inline-block" onsubmit="return confirm('Delete image?');">
                                        @csrf
                                        @method('DELETE')
                    ...
                    @empty
                        <tr>
                            <td colspan="5" class="px-6 py-12 text-center text-slate-500 font-medium">No media found.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
    ...
    <script>
        function setViewMode(mode) {
            ...
            localStorage.setItem('admin_gallery_view', mode);
        }

        document.addEventListener('DOMContentLoaded', () => {
            const savedMode = localStorage.getItem('admin_gallery_view') || 'grid';
