...
@section('content')
    <div class="max-w-5xl mx-auto space-y-6">
        ...
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-2xl font-bold text-slate-900">Edit Program: {{ $program->name }}</h1>
                <p class="text-slate-600 text-sm">Update program configuration and settings.</p>
            </div>
            <div class="flex gap-2">
                <a href="{{ route('admin.programs.show', $program->id) }}"
                    class="inline-flex items-center gap-2 px-4 py-2 bg-white border border-slate-200 text-[#0B4D73] rounded-xl text-sm font-semibold hover:bg-slate-50 transition-colors shadow-sm">
        ...
        <form action="{{ route('admin.programs.update', $program->id) }}" method="POST" enctype="multipart/form-data"
            class="space-y-6" id="editProgramForm">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div class="lg:col-span-1 space-y-6">
                    ...
                    <div class="glass rounded-2xl p-6">
                        ...
                        <div class="space-y-3">
                            @php $currentType = old('type', $program->type); @endphp
                            <label class="relative block cursor-pointer group">
                                <input type="radio" name="type" value="rolling" class="peer sr-only" {{ $currentType == 'rolling' ? 'checked' : '' }} onchange="toggleTypeFields()">
                                <div
                            ...
                            <label class="relative block cursor-pointer group">
                                <input type="radio" name="type" value="scheduled" class="peer sr-only" {{ $currentType == 'scheduled' ? 'checked' : '' }} onchange="toggleTypeFields()">
                                <div
                            ...
                            <label class="relative block cursor-pointer group">
                                <input type="radio" name="type" value="journey" class="peer sr-only" {{ $currentType == 'journey' ? 'checked' : '' }} onchange="toggleTypeFields()">
                                <div
                                    class="p-4 rounded-xl border-2 border-slate-100 peer-checked:border-rose-500 peer-checked:bg-rose-50 transition-all">
                                    <div class="flex items-center gap-3">
                                        <div
                                            class="w-10 h-10 rounded-full bg-rose-100 text-rose-600 flex items-center justify-center">
                            ...
                            <label class="relative block cursor-pointer group">
                                <input type="radio" name="type" value="offline" class="peer sr-only" {{ $currentType == 'offline' ? 'checked' : '' }} onchange="toggleTypeFields()">
                                <div
                        ...
                        <p class="text-[10px] text-slate-400 mt-4 leading-tight italic">
                            <i class="fas fa-info-circle"></i> Changing the program type may clear specific age/date
                            configurations.
                        </p>
                    </div>
                        ...
                        <div class="space-y-4">
                            ...
                            <div class="pt-2">
                                <label class="flex items-center gap-3 cursor-pointer group">
                                    <div class="relative">
                                        <input type="checkbox" name="is_featured" value="1" {{ old('is_featured', $program->is_featured) ? 'checked' : '' }} class="peer sr-only">
                                        <div
                                            class="w-10 h-6 bg-slate-200 rounded-full peer peer-checked:bg-amber-400 transition-colors">
                                        </div>
                                        <div
                        ...
                        <div class="space-y-5">
                            ...
                            <div class="space-y-1">
                                ...
                                <textarea name="description" rows="5"
                                    ...
                                    placeholder="What is this program about?">{{ old('description', $program->description) }}</textarea>
                    ...
                    <div class="glass rounded-2xl p-6">
                        ...
                        <div id="rollingFields" class="space-y-5">
                            <div class="space-y-1 max-w-xs">
                                ...
                                <div class="relative">
                                    <input type="number" name="age_target"
                                        value="{{ old('age_target', $program->age_target) }}"
                        ...
                        <div id="scheduledFields" class="hidden space-y-6">
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                                <div class="space-y-1">
                                    ...
                                    <input type="number" name="cohort_age_min"
                                        value="{{ old('cohort_age_min', $program->cohort_age_min) }}"
                                ...
                                <div class="space-y-1">
                                    ...
                                    <input type="number" name="cohort_age_max"
                                        value="{{ old('cohort_age_max', $program->cohort_age_max) }}"
                                        class="w-full px-4 py-2.5 bg-white/50 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-[#0B4D73] focus:bg-white transition-all"
                                        placeholder="e.g., 18">
                                </div>
                            </div>
                            ...
                            <div id="dateFields" class="grid grid-cols-1 md:grid-cols-2 gap-5">
                                <div class="space-y-1">
                                    ...
                                    <input type="date" name="start_date"
                                        value="{{ old('start_date', $program->start_date ? \Carbon\Carbon::parse($program->start_date)->format('Y-m-d') : '') }}"
                                        class="w-full px-4 py-2.5 bg-white/50 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-[#0B4D73] focus:bg-white transition-all">
                                </div>
                                <div class="space-y-1">
                            ...
                            <div id="pricingMediaFields" class="pt-4 border-t border-slate-100">
                                <h4 class="text-xs font-bold uppercase tracking-wider text-slate-400 mb-4">Pricing & Media
                                </h4>
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-5 mb-5">
                                    <div class="space-y-1">
                                        <label
                                            class="text-sm font-semibold text-slate-700 flex items-center justify-between">
                                            <span>Price (₦)</span>
                                            <label class="flex items-center gap-2 cursor-pointer">
                                                <input type="checkbox" name="is_free" value="1" {{ old('is_free', $program->is_free) ? 'checked' : '' }}
                                    ...
                                    <div class="space-y-1">
                                        <label class="text-sm font-semibold text-slate-700">YouTube Jingle / Advert
                                            URL</label>
                                        <input type="url" name="youtube_url"
                                            value="{{ old('youtube_url', $program->youtube_url) }}"
                                            class="w-full px-4 py-2.5 bg-white/50 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-[#0B4D73] focus:bg-white transition-all"
                                ...
                                <div class="space-y-4">
                                    @if($program->thumbnail_path)
                                        <div
                                            class="relative w-48 aspect-video rounded-xl overflow-hidden border border-slate-200 group">
                                            <img src="{{ asset('storage/' . $program->thumbnail_path) }}"
                                                class="w-full h-full object-cover">
                                            <div
                                                class="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                                                <p class="text-white text-[10px] font-bold">Current Thumbnail</p>
                                            </div>
                                        </div>
                                    @endif
                                    <div class="space-y-1">
                                        <label class="text-sm font-semibold text-slate-700">Update Thumbnail Image</label>
