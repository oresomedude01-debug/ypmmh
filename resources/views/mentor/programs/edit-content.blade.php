...
@section('title', 'Edit Content - ' . $program->name)

@section('content')
    <div class="max-w-4xl mx-auto animate-fade-in">
        <!-- Header -->
        <div class="mb-8">
            <a href="{{ route('mentor.programs.show', $program->id) }}"
                class="inline-flex items-center gap-2 text-slate-600 hover:text-[#0B4D73] transition-colors mb-6">
                <i class="fas fa-arrow-left"></i>
                <span class="font-medium">Back to Program</span>
            </a>
            <h1
        ...
        <div class="bg-white rounded-3xl shadow-lg border border-slate-100">
            <div class="p-8 border-b border-slate-100 bg-slate-50/50 rounded-t-3xl">
                <div class="flex items-center gap-4">
                    <div
                        ...
                        <i
                            class="fas {{ in_array($content->content_type, ['video', 'video_pdf']) ? 'fa-video' : 'fa-file-pdf' }}"></i>
                    </div>
                    <div>
                        ...
                        <span class="text-sm font-medium text-slate-500 uppercase tracking-wider">
                            @if($content->content_type === 'video_pdf')
                                Video & PDF Lesson
                            @elseif($content->content_type === 'pdf_only' || $content->content_type === 'pdf')
                                PDF Activity
                            @else
                                Video Content
                            @endif
            ...
            <form action="{{ route('mentor.programs.contents.update', [$program->id, $content->id]) }}" method="POST"
                    ...
                    @else
                        ...
                        <div class="space-y-1 md:col-span-2">
                            ...
                            <input type="date" name="publish_at"
                                value="{{ old('publish_at', substr($content->publish_at ?? '', 0, 10)) }}" required
                                class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-[#0B4D73]">
                        </div>
                    @endif
                </div>
                ...
                <div class="flex gap-4 pt-6 border-t border-slate-100">
                    <a href="{{ route('mentor.programs.show', $program->id) }}"
                        class="flex-1 px-6 py-3 border border-slate-200 text-slate-600 font-bold rounded-xl hover:bg-slate-50 transition-colors text-center">
                        Cancel
                    </a>
                    <button type="submit"
            ...
            <!-- Report Button -->
            <div class="p-8 border-t border-slate-100 bg-amber-50/50 rounded-b-3xl">
                <div class="flex items-center justify-between">
                    <div>
                        <h4 class="font-bold text-amber-700">Report Content Issue</h4>
                        <p class="text-sm text-amber-600">Found an issue with this content? Report it to admins.</p>
                    </div>
                    <button type="button" onclick="openReportModal()"
                        class="px-6 py-3 bg-amber-500 text-white font-bold rounded-xl hover:bg-amber-600 transition-all inline-flex items-center gap-2">
