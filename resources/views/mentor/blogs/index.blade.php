    ...
    <div class="space-y-8 animate-fade-in">
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
                ...
                <p class="font-medium" style="color: var(--text-secondary);">Manage and track the performance of your articles.</p>
            </div>
            <div class="flex items-center gap-3">
                ...
                <a href="{{ route('mentor.blogs.create') }}" class="btn btn-primary shadow-lg shadow-[#0B4D73]/20">
        ...
        <div class="grid grid-cols-2 md:grid-cols-2 lg:grid-cols-4 gap-6">
            <div class="analytics-card rounded-2xl p-6 relative overflow-hidden group">
                <div class="relative z-10">
                    ...
                    <p class="text-[10px] sm:text-sm font-bold uppercase tracking-wider mb-1" style="color: var(--text-secondary);">Your Posts</p>
                ...
                @foreach($posts as $post)
                    <div class="glass rounded-3xl overflow-hidden border border-slate-100/50 hover:shadow-xl hover:-translate-y-1 transition-all duration-300 group flex flex-col">
                        ...
                        <div class="p-4 bg-slate-50/50 border-t border-slate-100 grid grid-cols-3 gap-2">
                            ...
                            <a href="{{ route('mentor.blogs.edit', $post->id) }}"
                            ...
                            <form action="{{ route('mentor.blogs.destroy', $post->id) }}" method="POST"
                            ...
                            <tr class="hover:bg-slate-50/50 transition-colors group">
                                <td class="px-6 py-4">
                                    <div class="flex items-center gap-4">
                                        ...
                                        <div>
                                            ...
                                            <p class="text-[10px] text-slate-400 font-medium">{{ $post->category }} • {{ $post->created_at->format('M d, Y') }}</p>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-6 py-4">
                                ...
                                <td class="px-6 py-4 text-right">
                                    <div class="flex items-center justify-end gap-2">
                                        ...
                                        <a href="{{ route('mentor.blogs.edit', $post->id) }}" class="p-2 text-slate-400 hover:text-amber-500 hover:bg-amber-50 rounded-lg transition-all">
                                            <i class="fas fa-edit text-sm"></i>
                                        </a>
                                        <form action="{{ route('mentor.blogs.destroy', $post->id) }}" method="POST" class="inline-block" onsubmit="return confirm('Delete post?');">
                                            @csrf
                                            @method('DELETE')
        ...
        <script>
            function setViewMode(mode) {
                ...
                localStorage.setItem('mentor_blogs_view', mode);
            }

            document.addEventListener('DOMContentLoaded', () => {
                const savedMode = localStorage.getItem('mentor_blogs_view') || 'table';
